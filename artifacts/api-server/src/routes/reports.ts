...
router.get("/reports/export", authFlexible, async (req: AuthRequest, res) => {
  ...
  const records =
    fleetIds.length > 0
      ? await db
          ...
          .where(
            and(
              gte(fareRecordsTable.createdAt, since),
              inArray(fareRecordsTable.fleetId, fleetIds),
            ),
          )
  ...
  const summary = {
    ...
    "Regular Passengers": rows.filter((r) => r["Passenger Type"] === "regular")
      .length,
    "Student Passengers": rows.filter((r) => r["Passenger Type"] === "student")
      .length,
    "Senior Passengers": rows.filter((r) => r["Passenger Type"] === "senior")
      .length,
  };
  ...
  const wsRecords = XLSX.utils.json_to_sheet(
    rows.length ? rows : [{ Note: "No records in range" }],
  );
  ...
  if (format === "csv") {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="beepjeep-report-${Date.now()}.csv"`,
    );
    res.send(csv);
    return;
  }
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="beepjeep-report-${Date.now()}.xlsx"`,
  );
  res.send(buf);
});
...
router.get(
  ...
  async (req: AuthRequest, res) => {
      ...
      fleetIds.length > 0
        ? (
            await db
              ...
              .where(
                and(
                  inArray(usersTable.fleetId, fleetIds),
    ...
    res.json({
      totalFareToday: records.reduce(
        (s, r) => s + parseFloat(String(r.amount)),
        0,
      ),
