...
router.get(
  "/fare-settings",
  requireRole("fleet_driver", "independent_driver", "admin"),
  async (req: AuthRequest, res) => {
    ...
    if (user.role === "independent_driver") {
      const [setting] = await db
        ...
        .where(
          and(
            eq(fareSettingsTable.ownerId, user.id),
            eq(fareSettingsTable.ownerType, "driver"),
          ),
        )
        .limit(1);
    ...
    if (user.role === "fleet_driver" && user.fleetId) {
      const [setting] = await db
        ...
        .where(
          and(
            eq(fareSettingsTable.ownerId, user.fleetId),
            eq(fareSettingsTable.ownerType, "fleet"),
          ),
        )
        .limit(1);
      if (!setting) return res.json(defaults);
    ...
    if (!isNaN(fleetId)) {
      const [setting] = await db
        ...
        .where(
          and(
            eq(fareSettingsTable.ownerId, fleetId),
            eq(fareSettingsTable.ownerType, "fleet"),
          ),
        )
        .limit(1);
...
router.put(
  "/fare-settings",
  requireRole("independent_driver", "admin"),
  async (req: AuthRequest, res): Promise<void> => {
    const user = req.user!;
    const { regularFare, studentFare, seniorFare, fleetId } = req.body ?? {};

    if (
      regularFare === undefined ||
      studentFare === undefined ||
      seniorFare === undefined
    ) {
      res
        .status(400)
        .json({ error: "regularFare, studentFare, seniorFare are required" });
      return;
    }
    ...
    if (user.role === "independent_driver") {
      ...
      res.json({ regularFare, studentFare, seniorFare });
      return;
    }
    ...
    if (user.role === "admin" && fleetId) {
      const fleet = await db
        ...
        .where(
          and(
            eq(fleetsTable.id, Number(fleetId)),
            eq(fleetsTable.adminId, user.id),
          ),
        )
        .limit(1);
      ...
      const existing = await db
        ...
        .where(
          and(
            eq(fareSettingsTable.ownerId, Number(fleetId)),
            eq(fareSettingsTable.ownerType, "fleet"),
          ),
        )
        .limit(1);
